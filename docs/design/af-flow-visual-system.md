...
- **Full-bleed square master, system does the masking.** This Mac runs the macOS generation (Darwin 25) that masks app icons into its own squircle. The canon's rule 4 logic (where the system applies its own mask, supply full bleed with square corners, because transparent rounded corners render wrongly under a mask) therefore extends to macOS. The master is a 1024x1024 square with the plate colour `#16161A` filling the entire canvas, no margin, no rounded corners of our own, no hairline (canon rule 3: only the website header placement carries one).
...
- **If the app ever targets older macOS** that does not mask: generate the classic variant instead, plate 824x824 centred on a transparent 1024 canvas, corner radius 180 (the canon's rx=14/64 scaled), same construction scaled by 12.875. This is the recorded alternative, not the shipped one.
...
- The menu bar status item gets the template monogram per 5.9. The sidebar lockup (5.1) reuses the same construction at 28pt as a pre-rendered asset.

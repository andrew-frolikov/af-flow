...
**2026-09-07, Phase 4 built.** `AF Flow Models.xpc` ships inside the app with
exactly `app-sandbox` and `network.client`; the app still has neither. Speech
models are pinned per file at fixed Hub commits (open item 2, closed), 47 files
across the two rungs, generated by `scripts/speech-model-pins.py` from the tree
API. `ModelDownloader` opens the destination and hands the descriptor over, so
the service writes into a file it could not open itself, and the bytes are
checked by hash and size where they land. `scripts/model-download-check.sh`
fetches the smallest real pinned file over HTTPS end to end and verifies it.

**AND ONE EXPECTATION THAT DID NOT HAPPEN, recorded because it is load-bearing
for item 3.** The plan said the first real download would earn a LuLu prompt and
therefore a rule for `com.frolikov.afflow.models`, and that this rule is what
the checker's one exception exists to permit. It did not. After a real HTTPS
fetch by the service, LuLu's database still held 241 rules and 3 for the family,
none of them the downloader's, and no prompt appeared. LuLu is enabled,
`passiveMode` false, `blockMode` false; its `allowInstalled` and `allowApple`
preferences are true. **Why no rule was created is NOT established here**, and
the difference matters: if a shipping, notarized copy on a friend's Mac does
prompt, the exception is doing real work; if nothing ever prompts, the exception
is dormant. Either way it is safe, because it only ever excuses a rule that
exists and only while a bundle carrying that identifier ships. Worth one look
when the first notarized DMG runs on a machine with LuLu.

...
extension SpeechModelDescriptor {
    /// Every file this model needs, pinned by hash and size, or nil for the
    /// backends this project does not download.
    ///
    /// FluidAudio and SpeechAnalyzer fetch their own models by their own means,
    /// and claiming to pin them would be a claim the code cannot keep. For
    /// WhisperKit this covers BOTH folders a model lives in: the Core ML files
    /// and the tokenizer. Generated by `scripts/speech-model-pins.py`.

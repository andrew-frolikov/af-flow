...
final class GeneratedWikiEngine {
    ...
    private func ensureWikiRoot() throws {
        ...
        if !FileManager.default.fileExists(atPath: root.appendingPathComponent(".git").path) {
            ...
            _ = runGit(["config", "user.name", "AFFlow Wiki"], in: root)
            _ = runGit(["config", "user.email", "wiki@afflow.local"], in: root)
        }
    }
    ...
    private func writePage(url: URL, type: String, name: String, extraFrontmatter: [String: String], body: String) throws {
        ...
        let incomingBodyHash = AFFlowHistoryStore.stableHash(incomingBody)
        ...
        let existingBodyHash = existingBody.map(AFFlowHistoryStore.stableHash)
        ...
        let finalBodyHash = AFFlowHistoryStore.stableHash(finalBody)

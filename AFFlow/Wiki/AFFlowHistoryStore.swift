...
enum AFFlowHistoryActor: String, Codable {
    case user
    case afFlow = "ghost_pepper"
}

struct AFFlowHistoryEvent: Codable, Equatable {
    var id: UUID = UUID()
    var createdAt: Date = Date()
    var actor: AFFlowHistoryActor
...
enum AFFlowHistoryStore {
    static let metadataFolderName = ".afflow"
    static let historyFolderName = "history"

    static func recordEvent(
        archiveRoot: URL,
        actor: AFFlowHistoryActor,
        operation: String,
        summary: String,
        relativePath: String,
        ...
        do {
            ...
            NSLog("AFFlowHistoryStore: failed to record event %@ for %@: %@", operation, relativePath, error.localizedDescription)
        }
    }
    ...
    static func recordFileChange(
        ...
        do {
            ...
            NSLog("AFFlowHistoryStore: failed to record history for %@: %@", relativePath, error.localizedDescription)
        }
    }
    ...
    private static func append(_ event: AFFlowHistoryEvent, archiveRoot: URL) throws {

...
final class TextCleanupManager: ObservableObject, TextCleaningManaging {
    ...
    private static let systemPromptSentinel = "<|af-flow-system-prefill-split|>"
    private static let userInputSentinel = "<|af-flow-user-prefill-split|>"
    private static let repositoryDownloadMarkerFileName = ".af-flow-model-cache-complete"

    /// The marker written before the app was renamed. A cache carrying only this
    /// one is complete and must still count as complete: treating it as missing
    /// would re-download several gigabytes of weights that are already on disk.
    private static let legacyRepositoryDownloadMarkerFileName = ".ghostpepper-model-cache-complete"
    ...
    private static func isVerifiedRepository(_ url: URL) -> Bool {
        ...
        if fileManager.fileExists(atPath: url.appendingPathComponent(repositoryDownloadMarkerFileName).path) {
            return true
        }
        return fileManager.fileExists(
            atPath: url.appendingPathComponent(legacyRepositoryDownloadMarkerFileName).path
        )
    }

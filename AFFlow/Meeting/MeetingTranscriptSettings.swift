...
enum MeetingTranscriptSettings {
    ...
    /// Returns the default save directory: Documents/AF Flow Meetings.
    ...
    static func appContainerArchiveIfPresent() -> URL? {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.frolikov.afflow"
        let archive = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent("Library/Containers", isDirectory: true)
            .appendingPathComponent(bundleID, isDirectory: true)
            .appendingPathComponent("Data/Documents/\(archiveFolderName)", isDirectory: true)
    ...
    static let archiveFolderName = "AF Flow Meetings"

    /// The folder name used before the app was renamed.
    ///
    /// Transcripts already written are NOT moved. They sit in a folder the user
    /// can see and may have bookmarked, and a rename under them is a surprise,
    /// not a migration. Instead the old folder keeps being used when it is the
    /// one that exists, so nothing a previous version wrote goes missing.
    static let legacyArchiveFolderName = "Ghost Pepper Meetings"

    private static func documentsArchiveURL() -> URL {
        let documents = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let current = documents.appendingPathComponent(archiveFolderName, isDirectory: true)
        let legacy = documents.appendingPathComponent(legacyArchiveFolderName, isDirectory: true)
    ...
    /// True for either archive folder, current or pre-rename.
    private static func isOwnSandboxArchiveURL(_ url: URL) -> Bool {
        guard isRunningInAppSandbox else { return false }
        let documents = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let path = url.standardizedFileURL.path
        return [archiveFolderName, legacyArchiveFolderName].contains { name in
            documents.appendingPathComponent(name, isDirectory: true).standardizedFileURL.path == path
        }
    }
    ...
    private static func isLegacyAppContainerArchiveURL(_ url: URL) -> Bool {
        guard !isRunningInAppSandbox else { return false }
        let path = url.standardizedFileURL.path
        guard path.contains("/Library/Containers/") else { return false }
        return [archiveFolderName, legacyArchiveFolderName].contains { name in
            path.hasSuffix("/Data/Documents/\(name)")
        }
    }
}

...
def suite_failures():
    ...
    if not totals:
        return False, 0, []
    ran = max(int(t[0]) for t in totals)
    failures = max(int(t[1]) for t in totals)
...
def main():
    ...
    ran, baseline, baseline_names = suite_failures()
    if not ran:
        print("the suite did not run. Is AF Flow open?")
        return 1
    print("  %d tests, %d pre-existing failure(s)" % (ran, baseline))
    print()

    killed, survived, broken = [], [], []
    try:
        for index, (name, path, old, new, why) in enumerate(MUTATIONS, 1):
            full = os.path.join(REPO, path)
            source = open(full, encoding="utf-8").read()
            if old not in source:
                print("[%d/%d] %-28s SKIPPED, the target text is gone"
                      % (index, len(MUTATIONS), name))
            ...
            if not ran:
                print("[%d/%d] %-28s DID NOT COMPILE" % (index, len(MUTATIONS), name))
                ...
                print("[%d/%d] %-28s KILLED by %d test(s)"
                      % (index, len(MUTATIONS), name, len(new_names)))
                killed.append((name, why, new_names))
            else:
                print("[%d/%d] %-28s *** SURVIVED ***" % (index, len(MUTATIONS), name))
